...
if nargin < 6
    ...
    loghyperGP = minimize_vhgpr(loghyperGP, 'gpr_vhgpr', 40, {'covSum', {'covSEardj','covNoise','covConst'}}, x_tr, y_tr);
end
...
if nargin < 7 % Learn hyperparameters
    ...
    [LambdaTheta, convergence0] = minimize_vhgpr(LambdaTheta, 'vhgpr', 30, covfuncSignal, covfuncNoise, 2, x_tr, y_tr);
    display('Running VHGPR...')
    [LambdaTheta, convergence] = minimize_vhgpr(LambdaTheta, 'vhgpr', iter, covfuncSignal, covfuncNoise, 0, x_tr, y_tr);
    convergence = [convergence0; convergence];
else % Use given, fixed hyperparameters
    display('Running VHGPR...')
    LambdaTheta = [log(0.5)*ones(n,1);loghyper];
    [LambdaTheta, convergence] = minimize_vhgpr(LambdaTheta, 'vhgpr', iter, covfuncSignal, covfuncNoise, 2, x_tr, y_tr);
end
...
%     [NLPDapprox, NLPD, Testmu] = GH_quadrature_test(y_tst, mutst, diagSigmatst, atst, diagCtst);
